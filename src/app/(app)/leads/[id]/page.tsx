      ...
      <section className="lead-detail-grid">
        <div className="lead-main-stack">
          <section className="panel social-overview-panel">
            <div className="panel-heading"><div><p className="eyebrow">Kontak bisnis</p><h2>Website & social media</h2><p className="panel-description">Gunakan bagian ini untuk cek profil bisnis sebelum mengirim pesan.</p></div><span className={lead.social?.verificationStatus?.startsWith("Verified") ? "verification verified" : "verification needs-check"}>{lead.social?.verificationStatus || lead.socialVerification || "Belum diverifikasi"}</span></div>
          ...
          <section className="panel">
            <div className="panel-heading"><div><p className="eyebrow">Ringkasan riset</p><h2>Kenapa bisnis ini layak dihubungi?</h2><p className="panel-description">Baca bagian ini kalau butuh konteks sebelum chat atau meeting.</p></div><span className="score-large">{lead.opportunityScore}<small>/100</small></span></div>
            ...
            <div className="research-grid">
              ...
              <ResearchBlock label="Offer yang cocok" text={lead.recommendedOffer} />
            ...
            <div className="guardrail"><strong>Catatan penting:</strong> {lead.guardrail}</div>
          ...
          <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Riwayat lead</p><h2>Aktivitas terakhir</h2></div></div><ActivityTimeline activities={activities} /></section>
        </div>

        <aside className="lead-side-stack">
          <section className="panel sticky-panel">
            <p className="eyebrow">Update progress</p>
            <label className="field"><span>Status lead</span><select value={lead.stage} onChange={(e) => changeLeadStage(lead, e.target.value as LeadStage)}>{LEAD_STAGES.map((stage) => <option key={stage} value={stage}>{stage}</option>)}</select></label>
            ...
            <div className="detail-list">
              ...
              <div><span>Channel utama</span><strong>{lead.contactRoute || "—"}</strong></div>
              <div><span>Kontak terakhir</span><strong>{formatDate(lead.lastContactAt)}</strong></div>
              <div><span>Follow-up berikutnya</span><strong>{formatDate(lead.nextFollowUpAt)}</strong></div>
            </div>
