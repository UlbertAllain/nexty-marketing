...
export const statusLabel:Record<LeadStatus,string>={NEW:"Belum dihubungi",READY_TO_CONTACT:"Pesan sudah dibuka",CONTACTED:"Sudah dikirim",WAITING_REPLY:"Menunggu jawaban",REPLIED:"Sudah membalas",QUALIFIED:"Peluang cocok",MEETING:"Jadwal pertemuan",PROPOSAL:"Penawaran dikirim",NEGOTIATION:"Sedang negosiasi",DEAL:"Berhasil jadi klien",NOT_INTERESTED:"Belum berminat",LOST:"Peluang berhenti"};
const transitions:Record<LeadStatus,LeadStatus[]>={NEW:["READY_TO_CONTACT","CONTACTED","NOT_INTERESTED"],READY_TO_CONTACT:["CONTACTED","NEW","NOT_INTERESTED"],CONTACTED:["WAITING_REPLY","REPLIED","NOT_INTERESTED"],WAITING_REPLY:["REPLIED","NOT_INTERESTED","LOST"],REPLIED:["QUALIFIED","NOT_INTERESTED","LOST"],QUALIFIED:["MEETING","PROPOSAL","NOT_INTERESTED","LOST"],MEETING:["PROPOSAL","NEGOTIATION","NOT_INTERESTED","LOST"],PROPOSAL:["NEGOTIATION","DEAL","LOST"],NEGOTIATION:["DEAL","LOST"],DEAL:[],NOT_INTERESTED:["READY_TO_CONTACT"],LOST:["READY_TO_CONTACT"]};
export const allowedTransitions=(status:LeadStatus)=>transitions[status];
export function assertTransition(from:LeadStatus,to:LeadStatus){if(from!==to&&!transitions[from].includes(to))throw new Error(`Perkembangan “${statusLabel[from]}” belum bisa langsung dipindahkan ke “${statusLabel[to]}”.`)}
export const normalizeCompany=(value:string)=>value.trim().toLocaleLowerCase("id-ID").replace(/\s+/g," ");
export function normalizePhone(value:string){let phone=value.replace(/[^\d+]/g,"");if(phone.startsWith("+"))phone=phone.slice(1);if(phone.startsWith("0"))phone=`62${phone.slice(1)}`;return phone}
export const isValidWhatsApp=(value:string)=>/^62[1-9]\d{7,12}$/.test(normalizePhone(value));
export function validateLead(input:Partial<Lead>){const errors:string[]=[];if(!input.companyName?.trim())errors.push("Nama perusahaan belum diisi.");if(input.email&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(input.email))errors.push("Alamat email belum sesuai format.");if(input.phone&&!isValidWhatsApp(input.phone))errors.push("Nomor WhatsApp belum sesuai format Indonesia.");return errors}
export function generateMessage(template:Pick<Template,"content">,lead:Pick<Lead,"companyName"|"contactName"|"category">){return template.content.replaceAll("{{company_name}}",lead.companyName).replaceAll("{{contact_name}}",lead.contactName||"Bapak/Ibu").replaceAll("{{category}}",lead.category||"bisnis")}
export function whatsappDraftUrl(phone:string,message:string){const normalized=normalizePhone(phone);if(!isValidWhatsApp(normalized))throw new Error("Nomor WhatsApp belum siap digunakan.");return `https://wa.me/${normalized}?text=${encodeURIComponent(message)}`}
