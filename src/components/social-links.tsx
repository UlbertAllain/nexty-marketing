...
export function SocialLinks({ social, compact = false }: { social?: SocialProfile; compact?: boolean }) {
  if (!social) return <span className="muted small">Belum ada profil sosial.</span>;
  ...
  const items: LinkItem[] = [
    ...
    social.websiteUrl ? { label: "Website", href: social.websiteUrl, icon: <Globe2 size={15} /> } : null,
    social.linkInBioUrl ? { label: "Link-in-bio", href: social.linkInBioUrl, icon: <LinkIcon size={15} /> } : null,
  ].filter(Boolean) as LinkItem[];

  if (!items.length) return <span className="muted small">Belum ada channel non-WA yang terverifikasi.</span>;
